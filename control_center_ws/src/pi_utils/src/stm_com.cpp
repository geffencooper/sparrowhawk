...
void follow_object()
{
	...
	motor_msg.drive_value = 80;
}

#define BREAK_PERIOD 5 // reverse for 50 frames
#define OUT_OF_VIEW_PERIOD 10 // wait 100 frames before reversing
#define BACKWARD_PERIOD 15 // reverse for 100 frames
void tracker_callback(const geometry_msgs::Point::ConstPtr& p)
{
	  ...
	  switch(car_state)
	  {
		  ...
		  case(BACKWARD):
		  {
			  ...
			  {
				  ROS_INFO("backward count [%i]", oov_count);
				  backward_count++;
			  }
			  break;
		  }
